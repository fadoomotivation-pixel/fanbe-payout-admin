...
import { Search, UserCheck, UserPlus, RefreshCw, MoreVertical, Shield, ShieldOff } from 'lucide-react'
...
const RANK_LABELS = {
  ...
  senior_sales_officer: 'Sr. Sales Officer',
  team_leader: 'Team Leader',
  senior_team_leader: 'Sr. Team Leader',
  team_manager: 'Team Manager',
  senior_team_manager: 'Sr. Team Manager',
  area_manager: 'Area Manager',
  senior_area_manager: 'Sr. Area Manager',
  regional_manager: 'Regional Manager',
  senior_regional_manager: 'Sr. Regional Manager',
...
export default function BrokersList() {
  ...
  const [actionMenu, setActionMenu] = useState(null) // broker id with open menu
  ...
  async function fetchBrokers() {
    ...
    const { data } = await supabase
      .from('brokers')
      .select('id, name, mobile, phone, email, rank, status, kyc_status, portal_access, last_login, city, created_at')
  ...
  async function togglePortalAccess(broker) {
    ...
    toast.success(`Portal access ${newVal ? 'enabled' : 'disabled'} for ${broker.name}`)
  ...
  async function resetPassword(broker) {
    ...
    toast.success(`New temp password: ${newPwd} — Share with broker manually`, { duration: 8000 })
    setActionMenu(null)
  }
  ...
  const filtered = useMemo(() => {
    ...
    if (search.trim()) {
      ...
      list = list.filter(b => [b.name, b.mobile, b.phone, b.email, b.city, b.rank].filter(Boolean).some(v => String(v).toLowerCase().includes(q)))
    }
    return list
  ...
  return (
    <div className="space-y-6" onClick={() => setActionMenu(null)}>
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
        ...
        <KpiCard title="Active" value={activeCount} color="green" />
        <KpiCard title="Portal Access" value={portalCount} color="blue" />
        <KpiCard title="KYC Verified" value={kycCount} color="teal" />
      </div>
      ...
      <div className="card p-4 flex flex-wrap gap-3 items-center">
        <div className="relative flex-1 min-w-[200px] max-w-xs">
          ...
          <input value={search} onChange={e => setSearch(e.target.value)} className="input pl-9" placeholder="Search name, mobile, city..." />
        </div>
          ...
          <table className="min-w-full text-sm">
            ...
            <tbody>
              ...
              {!loading && filtered.length === 0 && (
                <tr><td colSpan="8" className="px-4 py-10 text-center text-slate-500">No brokers found.</td></tr>
              )}
              {!loading && filtered.map(broker => (
                <tr key={broker.id} className="border-t border-slate-100 hover:bg-slate-50">
                  <td className="px-4 py-3">
                    ...
                    <div className="text-xs text-slate-400">{broker.city || broker.email || '—'}</div>
                  </td>
                  ...
                  <td className="px-4 py-3">
                    <Badge color={broker.kyc_status === 'verified' ? 'green' : broker.kyc_status === 'submitted' ? 'blue' : broker.kyc_status === 'rejected' ? 'red' : 'yellow'}>
                  ...
                  <td className="px-4 py-3">
                    <Badge color={(broker.status || 'active') === 'active' ? 'green' : 'gray'}>
                  ...
                  <td className="px-4 py-3 text-right">
                    <div className="flex items-center justify-end gap-2">
                      ...
                      <div className="relative" onClick={e => e.stopPropagation()}>
                        <button
                          onClick={() => setActionMenu(actionMenu === broker.id ? null : broker.id)}
                          className="p-1 rounded hover:bg-slate-200 text-slate-500">
                          <MoreVertical size={15} />
                        </button>
                        {actionMenu === broker.id && (
                          <div className="absolute right-0 top-7 z-20 bg-white border border-slate-200 rounded-xl shadow-lg w-48 py-1 text-sm">
                            ...
                            <button onClick={() => toggleStatus(broker)}
                              className="w-full text-left px-4 py-2 hover:bg-slate-50">
                              {broker.status === 'active' ? '🔴 Deactivate' : '🟢 Activate'}
                            </button>
                            <button onClick={() => resetPassword(broker)}
                              className="w-full text-left px-4 py-2 hover:bg-slate-50">🔑 Reset Password</button>
